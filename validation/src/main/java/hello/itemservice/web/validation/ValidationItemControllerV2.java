...
//    @PostMapping("/add")
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        if(bindingResult.hasErrors()) {
            ...
            // Model에 담지않아도 bindingResult가 뷰로 넘어감
            return "validation/v2/addForm";
        }
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if(resultPrice < 10000) {
                bindingResult.addError(new ObjectError("item",
                        null,null, "가격 * 수량의 합은 10,000원 이상이어야 합니다. 현재값 =" + resultPrice));
            }
        }
...
//    @PostMapping("/add")
    ...
    // reject, rejectValue 메서드로 코드 단순화
//    @PostMapping("/add")
    public String addItemV4(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {

        log.info("objectName={}", bindingResult.getObjectName()); // item
        log.info("target={}", bindingResult.getTarget()); // Item.toString

        // 검증로직
        // ValidationUtils.rejectIfEmptyOrWhitespace(bindingResult,"itemName","required"); // 아래코드와 동일
